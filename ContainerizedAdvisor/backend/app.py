...
FIREBASE_CREDENTIALS_PATH = os.getenv('FIREBASE_CREDENTIALS_PATH', 'firebae_key1.json')

# Data files
COURSE_DATA_FILE = os.getenv('COURSE_DATA_FILE', 'AllCourseRelatedData.json')
FACULTY_GENERAL_DATA_FILE = os.getenv('FACULTY_GENERAL_DATA_FILE', 'AllFacultyGeneralInformation.json')
FACULTY_RESEARCH_DATA_FILE = os.getenv('FACULTY_RESEARCH_DATA_FILE', 'AllFacultyResearchInformation.json')
...
def get_frontend_urls():
    if os.getenv('ENVIRONMENT') == 'PRODUCTION':
        return [
            os.getenv('PRODUCTION_FRONTEND_URL', 'https://advisorai.ai')
        ]
    else:
        url = os.getenv('FRONTEND_URLS', 'http://localhost:3000')
        return url
...
def read_csv_data(file_path: str) -> List[Dict[str, Any]]:
    ...
    if file_path == 'jobs.csv':
        file_path = JOBS_CSV_FILE
    elif file_path == 'internships.csv':
        file_path = INTERNSHIPS_CSV_FILE
    else:
