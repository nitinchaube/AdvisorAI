...
## 🔧 **Alternative Solution: Use OpenAI Instead**

If you're still having issues with Gemini, you can switch to OpenAI by updating your `.env` file:

```env:AdvisorAI/AdvisorAI-Web/backend/.env
# LLM Configuration
LLM_PROVIDER=openai
...
Let me also create a simple test script to verify the LLM service is working:

```python:AdvisorAI/AdvisorAI-Web/backend/test_llm.py
...
def test_llm_service():
    """Test the LLM service with a sample resume text"""
    try:
        ...
        result = llm_service.parse_resume(sample_text)
