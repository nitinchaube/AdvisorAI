...
const CompactSidebar = ({ activeTab, setActiveTab, sidebarOpen }) => {
  const menuItems = [
    {
      id: 'chat',
      icon: MessageCircle,
  ...
  return (
    <div className="h-full w-16 bg-white border-r-2 border-gray-300 shadow-lg flex flex-col z-30 relative">
      ...
      {/* Navigation Icons */}
      <div className={`flex-1 py-3 ${sidebarOpen ? 'pt-3' : 'pt-3'} bg-gray-50`}>
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Icon container */}
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-200 shadow-sm ${getColorClasses(item.color, isActive)}`}>
